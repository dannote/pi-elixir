...
With "table" — shows table info and contents. Use "match" for pattern matching and "limit" to cap rows.`,
		Type.Object({
			table: Type.Optional(Type.String({ description: "Table name (atom) or ID to inspect. Omit to list all tables." })),
			match: Type.Optional(Type.String({ description: 'Erlang match pattern for :ets.match_object, e.g. "{:_, :active, :_}" to match 3-tuples with :active as second element' })),
