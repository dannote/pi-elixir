...
const PROJECT_DIR = "/Users/dannote/Development/json_spec";
...
async function mcpCall(toolName: string, args: Record<string, unknown>): Promise<{ isError?: boolean; text: string }> {
	const id = ++mcpId;
...
describe.skipIf(!elixirAvailable)("embedded MCP server", () => {
	...
	beforeAll(async () => {
		await new Promise<void>((resolve, reject) => {
			const proc = spawn("mix", ["run", "--no-halt", SCRIPT_PATH, "--port", String(PORT)], {
				cwd: PROJECT_DIR,
	...
	it("project_eval evaluates 1 + 1", async () => {
		const result = await mcpCall("project_eval", { code: "1 + 1" });
		expect(result.isError).toBeFalsy();
		expect(result.text).toBe("2");
	});

	it("project_eval can access project modules", async () => {
		const result = await mcpCall("project_eval", {
	...
	it("get_source_location for JSONSpec returns a file path", async () => {
		const result = await mcpCall("get_source_location", {
			reference: "JSONSpec",
		});
		expect(result.isError).toBeFalsy();
	...
	it("get_docs for JSONSpec returns documentation", async () => {
		const result = await mcpCall("get_docs", { reference: "JSONSpec" });
		expect(result.isError).toBeFalsy();
	...
	it("get_logs returns log output", async () => {
		const result = await mcpCall("get_logs", { tail: 10 });
		expect(result.isError).toBeFalsy();
