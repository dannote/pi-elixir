...
describe("callTool", () => {
	...
	it("passes AbortSignal to fetch", async () => {
		vi.mocked(fetch).mockResolvedValueOnce(jsonResponse({ jsonrpc: "2.0", id: 1, result: { content: [] } }));
...
describe("resolveUrl", () => {
	...
	it("returns embedded URL when process is ready", async () => {
		...
		// Simulate readiness
		(fakeProc.stdout as EventEmitter).emit("data", Buffer.from("PI_MCP_READY on port 4041"));
